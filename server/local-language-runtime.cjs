...
function createLocalLanguageRuntime({model='qwen2.5-coder:7b-instruct',fetchImpl=globalThis.fetch,now=Date.now,timeoutMs=120000,diagnostics=false,contextTurns=[]}={}){
 ...
 async function generate(instruction,data,format,{signal,timeout=timeoutMs,stage='extraction'}={}){
  ...
  try{
   ...
   const hosted=model==='gpt-4.1-mini';
   if(inputBytes>(hosted?24000:expanded?6144:3072))throw Error('LOCAL_MODEL_INPUT_LIMIT');
   const outputBudget=hosted?(stage==='semantic-assessor'?Math.min(4096,Math.max(1536,(data.claims?.length??0)*80+256)):stage==='extraction'?3072:768):768;
   const body=JSON.stringify({model,system:instruction,prompt,format,stream:false,keep_alive:'5m',options:{temperature:0,num_ctx:expanded?8192:4096,num_predict:outputBudget}});
   if(trace)trace.requestBudget={inputBytes:Buffer.byteLength(instruction)+Buffer.byteLength(prompt),numCtx:expanded?8192:4096,numPredict:outputBudget,timeoutMs:timeout};
 ...
 const extractor={id:'ollama-universal-local',version:'1',async extract(source,options={}){
  ...
  if(output.unresolved.length){failures.push({stage:'extraction',code:'INTERPRETATION_UNRESOLVED',reasons:output.unresolved});throw Error('LOCAL_INTERPRETATION_UNRESOLVED');}
